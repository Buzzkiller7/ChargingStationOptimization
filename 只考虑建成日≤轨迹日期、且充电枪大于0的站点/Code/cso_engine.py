...
def station_cutoff_date(cfg=None):
    """Return the date whose station stock should be visible to the trajectory."""
    cfg = cfg or C.city()
    if cfg.get("trace_date"):
        return pd.Timestamp(cfg["trace_date"]).normalize()
    name = str(cfg.get("raw") or cfg.get("raw_path") or "")
...
def clean_stations(cfg=None, return_stats=False):
    """Load stations and keep only in-bbox, already-created, positive-capacity rows."""
    cfg = cfg or C.city()
    b = cfg["bbox"]
    sc = cfg["station_cols"]
    ...
    if sc.get("slow") in raw:
        slow = pd.to_numeric(raw[sc["slow"]], errors="coerce").fillna(0)
    else:
        slow = pd.Series(0, index=raw.index, dtype="float64")
    capacity = fast + slow

    coord_ok = lon.notna() & lat.notna()
    bbox_ok = lon.between(b["lon_min"], b["lon_max"]) & lat.between(b["lat_min"], b["lat_max"])
    capacity_ok = capacity > 0

    cutoff = station_cutoff_date(cfg)
    create_col = sc.get("create_time")
    if cutoff is not None and create_col in raw:
        created = pd.to_datetime(raw[create_col], errors="coerce")
        date_ok = created.notna() & (created.dt.normalize() <= cutoff)
        future_create_time = int((created.dt.normalize() > cutoff).sum())
        missing_create_time = int(created.isna().sum())
    else:
        date_ok = pd.Series(True, index=raw.index)
        future_create_time = 0
        missing_create_time = 0

    keep = coord_ok & bbox_ok & capacity_ok & date_ok
    st = raw.loc[keep].copy().reset_index(drop=True)
    kept_idx = keep[keep].index
    st["lon"] = lon.loc[kept_idx].to_numpy(dtype=np.float64)
    st["lat"] = lat.loc[kept_idx].to_numpy(dtype=np.float64)
    st["fast"] = fast.loc[kept_idx].to_numpy(dtype=np.int32)
    st["slow"] = slow.loc[kept_idx].to_numpy(dtype=np.int32)

    stats = dict(
        raw=int(len(raw)),
        kept=int(len(st)),
        dropped=int(len(raw) - len(st)),
        missing_coord=int((~coord_ok).sum()),
        out_of_bbox=int((coord_ok & ~bbox_ok).sum()),
        future_create_time=future_create_time,
        missing_create_time=missing_create_time,
        zero_capacity=int((capacity <= 0).sum()),
        cutoff_date="" if cutoff is None else str(cutoff.date()),
        cache_version=STATION_CACHE_VERSION,
    )
    return (st, stats) if return_stats else st


def precompute_stations(cfg=None):
    """Clean station stock and write the station cache used by optimization."""
    cfg = cfg or C.city()
    st, stats = clean_stations(cfg, return_stats=True)
    np.savez(
        C.cache_file("stations"),
        lon=st["lon"].to_numpy(dtype=np.float64),
        lat=st["lat"].to_numpy(dtype=np.float64),
        fast=st["fast"].to_numpy(dtype=np.int32),
        slow=st["slow"].to_numpy(dtype=np.int32),
        station_cache_version=np.int64(STATION_CACHE_VERSION),
        station_cutoff_date=np.array(stats["cutoff_date"]),
        raw_count=np.int64(stats["raw"]),
        kept_count=np.int64(stats["kept"]),
        dropped_count=np.int64(stats["dropped"]),
        dropped_future_create_time=np.int64(stats["future_create_time"]),
        dropped_zero_capacity=np.int64(stats["zero_capacity"]),
    )
    return stats


def _station_cache_is_current(z, cfg=None):
    cfg = cfg or C.city()
    cutoff = station_cutoff_date(cfg)
    cutoff_text = "" if cutoff is None else str(cutoff.date())
    names = set(z.files)
    if not {"lon", "lat", "fast", "slow", "station_cache_version", "station_cutoff_date"} <= names:
        return False
    version = int(np.asarray(z["station_cache_version"]).item())
    cached_cutoff = str(np.asarray(z["station_cutoff_date"]).item())
    return version == STATION_CACHE_VERSION and cached_cutoff == cutoff_text
...
def district_distribution(lf, rows=None, cfg=None, gps_sample_n=50_000, admin_gdf=None):
    ...
    station_work, station_stats = clean_stations(cfg, return_stats=True)

    point_gdf = gpd.GeoDataFrame(
        station_work,
        geometry=gpd.points_from_xy(station_work["lon"], station_work["lat"]),
        crs=4326,
    )
    try:
    ...
    return dict(
        ...
        station_filter=station_stats,
        boundary_note=boundary_note + f" 站点分区：GeoJSON 空间匹配；站点过滤保留 {station_stats['kept']}/{station_stats['raw']}。",
    )
...
def precompute_segments():
    ...
    station_stats = precompute_stations(cfg)
    
    return dict(city=cfg["key"], rows=int(n), vehicles=int(vc.max() + 1),
                stations=int(station_stats["kept"]), station_filter=station_stats)
...
def load_stations():
    path = C.cache_file("stations")
    if path.exists():
        z = np.load(path)
        if _station_cache_is_current(z):
            return z
        z.close()
    precompute_stations()
    return np.load(path)
