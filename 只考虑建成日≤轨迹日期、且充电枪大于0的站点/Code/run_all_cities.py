...
并把结果落盘到：  <项目根>/Figure/<city_key>_<trace_date>/

每个城市目录下生成：
  图（PNG）
    01_trajectory_vs_stations.png      轨迹范围 vs 充电站（仅过滤后站点）
  ...
  分析结果
    ...
    summary.txt                        人类可读汇总
    station_filter.csv                 站点过滤前后/丢弃明细（核心：只用建成且非零站）
...
----
    ...
    python run_all_cities.py --rebuild           # 强制重算缓存（数据/日期变更后用）
...
def step1(cfg, outdir: Path, metrics: dict, log: list):
    ...
    # ---- 1.2 站点供给：过滤前 vs 过滤后（核心：只保留建成且非零站） ----
    raw = pd.read_csv(cfg["stations_path"])
    fast_r = pd.to_numeric(raw[sc["fast"]], errors="coerce").fillna(0).sum()
    slow_r = (pd.to_numeric(raw[sc["slow"]], errors="coerce").fillna(0).sum()
              if sc.get("slow") in raw else 0)
    st, fs = E.clean_stations(cfg, return_stats=True)   # 与 STEP_3 优化同一套过滤
    fast_k = int(st["fast"].sum())
    slow_k = int(st["slow"].sum())
    metrics["station_filter"] = jsonable(dict(
        fs,
        guns_before_fast=int(fast_r), guns_before_slow=int(slow_r),
        guns_after_fast=fast_k, guns_after_slow=slow_k,
    ))
    pd.DataFrame([{
        "城市": cfg["label"], "轨迹日期cutoff": fs["cutoff_date"],
        "过滤前站点": fs["raw"], "过滤后站点": fs["kept"], "净丢弃": fs["dropped"],
        "晚于轨迹日": fs["future_create_time"], "缺create_time": fs["missing_create_time"],
        "零枪": fs["zero_capacity"], "越界": fs["out_of_bbox"], "缺坐标": fs["missing_coord"],
        "过滤前合计枪": int(fast_r + slow_r), "过滤后合计枪": fast_k + slow_k,
    }]).to_csv(outdir / "station_filter.csv", index=False, encoding="utf-8-sig")
    log.append(f"[s1  ] 站点过滤 {fs['raw']} -> {fs['kept']} "
               f"(晚于轨迹日 {fs['future_create_time']} | 零枪 {fs['zero_capacity']})")

    # ---- 1.3 轨迹范围 vs 站点（只画过滤后站点） ----
...
def step3(cfg, outdir: Path, metrics: dict, log: list, D):
    ...
    mE = w_exp > 0
    floorE = E.pigeonhole_decomposition(c[mE], w_exp[mE])["floor_frac"]
...
def write_summary(cfg, outdir: Path, metrics: dict, log: list):
    ...
    lines = [
        ...
        f"轨迹文件: {Path(cfg['raw']).name}   轨迹日期(cutoff): {cfg.get('trace_date')}",
        "=" * 60,
        "【站点过滤——只用建成且非零站】",
        f"  过滤前(原始表): {sf.get('raw')} 站",
        f"  过滤后(实际用): {sf.get('kept')} 站   (净丢弃 {sf.get('dropped')})",
        f"    晚于轨迹日 create_time: {sf.get('future_create_time')}",
        f"    缺 create_time: {sf.get('missing_create_time')} | 零枪: {sf.get('zero_capacity')}"
        f" | 越界: {sf.get('out_of_bbox')} | 缺坐标: {sf.get('missing_coord')}",
        "",
        "【需求与基线】",
...
def run_city(key: str, fig_root: Path, draws: int, rebuild: bool) -> dict:
    cfg = C.use_city(key)
    date = cfg.get("trace_date") or "NA"
    outdir = fig_root / f"{key}_{date}"
    outdir.mkdir(parents=True, exist_ok=True)
    metrics = {"city": key, "label": cfg["label"], "trace_date": date,
               "raw": cfg["raw"], "stations": cfg["stations"]}
    log: list = []
    print(f"\n=== {cfg['label']}  ->  {outdir}")

    # 缓存：数据/日期变更或 --rebuild 时强制重算（precompute 同时重建分段+站点缓存）
    if rebuild or not C.cache_file("segments").exists() or not C.cache_file("stations").exists():
        ...
        print("  复用已有缓存（load_stations 会自校验站点缓存版本/日期）。")
